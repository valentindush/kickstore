...
const NewArrival:FC<FeaturedProInterface> = (props) => {
  ...
  const theUnfilledStars: JSX.Element[] = rating < 5 ? [...Array(5 - Math.floor(rating))].map(
    (el: undefined, index: number) => {
      return <img src={unFilledStar} alt="" key={index} />;
    }
  ): [];

  return <div className="w-[20rem] h-[30rem] flex flex-col items-center justify-between">
    <div className="w-[76%] h-[65%] flex  flex-col items-start justify-between p-4 rounded-md relative" style={{ background: "linear-gradient(270deg, #FE7831 0%, #FFDEBE 100%)"}}>
      <span className="text-3xl text-white font-extrabold">0{id}</span>
      <img src={imgSource} alt="" className="absolute top-[-.5rem] right-[-5.5rem] max-w-none" />
    ...
    <div className="w-[85%] h-36 flex flex-col items-center justify-center gap-4">
      <div className="flex gap-2">{theRatingStars} {theUnfilledStars}</div>
